...
- [`pi-council`](skills/productivity/pi-council/SKILL.md)：经 pi CLI 将一个任务并行派发给多个不同大模型进行只读研讨（头脑风暴、方案评审、代码审核），并汇总各路观点为一份报告。
- [`skill-creator-portable`](skills/productivity/skill-creator-portable/SKILL.md)：创建、更新和审查 Agent Skills，校验规范 frontmatter 与客户端目标，并默认生成可退出的 OpenAI 元数据适配层。
