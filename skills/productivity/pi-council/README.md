...
以任务的主要交付物为准；用户明确指定时优先采用用户选择。

| Mode | 适用场景 |
| --- | --- |
| `discuss` | 形成有依据的立场并分析取舍；也是兜底模式 |
| `brainstorm` | 发散多样想法或明显不同的解决路径 |
| `review` | 评估已有方案、计划或设计，指出优点、风险、缺口并给出结论 |
| `code-review` | 检查现有代码的具体问题，并提供 `file:line` 证据 |
...
你的任务原文
   ...
   ▼
你确认：任务文本 + mode + thinking level + 派发阵容 + 项目 skill（确认前不派发）
   ▼
