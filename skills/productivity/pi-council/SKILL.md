...
council.py <task> [-t <task>]            # stdin works when no task arg is given
  ...
  --project-skill <name> ...             # explicit project-skill allowlist by frontmatter name;
                                         # repeatable, resolved under .pi/skills and .agents/skills
...
pi -p --mode json --tools read,grep,find,ls --no-session --no-approve \
   --model <provider/model> --thinking high [--skill <resolved-dir> ...] \
   [@focus-file ...]   # prompt fed via stdin
```
...
- `--no-session` keeps runs ephemeral. `--no-approve` excludes automatic project-resource discovery; user/global skill discovery remains enabled. A repeated `--project-skill <name>` is the narrow exception: the script resolves exact frontmatter names under workspace `.pi/skills` and `.agents/skills`, rejects missing or ambiguous names before dispatch, and loads only those skills. The read-only tool limit still applies.
...
- **Data egress**: the task, enabled project-skill paths and content, focus files, and anything lanes choose to read are sent to every provider in the lineup. Get the user's consent before pointing the council at confidential material.

## Output contract

stdout ends with machine-readable lines: `status=completed|partial|failed|config_required`, `output_path=`, `run_dir=`, `models=`, optional `project_skills=` and `failed=`, then `elapsed=`. `partial` means at least one lane failed while others succeeded. Exit codes: 0 = at least one lane succeeded (including partial), 1 = all lanes failed or bad input, 3 = lineup not configured (2 is reserved for argparse usage errors).

The run directory holds `<model-slug>.md` per lane plus the combined `result.md` (audit + status table + full opinions + optional synthesis). Each lane report is on disk before its `[council] <model> done|FAILED` message appears; the final report is written after all lanes finish and preserves lineup order. Every lane report records its requested thinking level, while the final audit table records the level for every council lane and optional synthesis model. These are configuration values only; no model thinking content is included. The final audit also records explicitly enabled project skills. Raw `<model-slug>.events.jsonl` streams are kept only for failed lanes, or for all lanes with `--events`. If every lane fails, synthesis is skipped and `result.md` says so explicitly. Reported token usage and cost are accumulated across every turn of a lane, including tool-call turns; `--thinking` is the main cost and latency lever.
